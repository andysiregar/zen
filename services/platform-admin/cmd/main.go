...
import (
	...
	"platform-admin/internal/config"
)
...
func main() {
	...
	_, err = database.NewDatabaseManager(database.Config{
	...
	{
		// Basic platform admin routes (require authentication)
		admin := v1.Group("/admin")
		admin.Use(middleware.AuthMiddleware(jwtService))
		{
			admin.GET("/status", func(c *gin.Context) {
				c.JSON(http.StatusOK, gin.H{
					"message": "Platform admin service is running",
					"service": "platform-admin",
				})
			})
		}
	...
	go func() {
		logger.Info("Starting Platform Admin service", zap.String("port", cfg.Server.Port))
	...
	logger.Info("Platform Admin service exited")
}